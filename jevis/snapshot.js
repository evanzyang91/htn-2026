(() => {
  ...
  // Sweep only controls the observation can see: on lazy pages, offscreen widgets stream in
  // continuously and must not invalidate decisions made on an identical visible state.
  cache.pageKey=()=>[performance.timeOrigin,location.href,scrollX,scrollY,innerWidth,innerHeight,
    [...document.querySelectorAll('input,textarea,select')].filter(e=>{
      if (!safe(e) || !visible(e)) return false;
  ...
  // Scroll whatever the user would scroll here. A dialog's option list scrolls on its own while
  // the page behind it is locked, so a document-only test offers no way to reach controls below
  // the fold inside it. Walk up from the viewport centre to the first scrollable ancestor.
  let box=null;
  for (let e=document.elementFromPoint(innerWidth>>1,innerHeight>>1);
       e && e!==document.body && e!==document.documentElement; e=e.parentElement) {
    if (e.scrollHeight>e.clientHeight+2 && /auto|scroll/.test(getComputedStyle(e).overflowY)) { box=e; break; }
  }
  if (box) {
    const r=box.getBoundingClientRect();
    const at={x:Math.min(innerWidth-1,Math.max(0,Math.round(r.x+r.width/2))),
              y:Math.min(innerHeight-1,Math.max(0,Math.round(r.y+r.height/2)))};
    ...
    if (box.scrollTop>0) actions.push({id:'scroll_up',kind:'scroll',label:'Scroll up',delta:-560,...at});
  } else {
    if (scrollY+innerHeight<height-2) actions.push({id:'scroll_down',kind:'scroll',label:'Scroll down',delta:560});
    if (scrollY>0) actions.push({id:'scroll_up',kind:'scroll',label:'Scroll up',delta:-560});
  }
  // Without this the agent cannot leave a page that has no control for its goal, such as a login
  ...
  return {url:location.href,title:document.title,w:innerWidth,h:innerHeight,text,loading,
